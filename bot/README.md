...
2. 창에 로블록스 닉네임을 입력하면, 봇이 `YCC-123` 형식(알파벳 3자 + 숫자 3자)의 코드를 컨테이너로 보내 줍니다.
3. 사용자가 그 코드를 로블록스 프로필 **소개란(About)** 에 붙여넣고 저장합니다.
4. 코드가 담긴 컨테이너 **안**에 있는 **인증 확인** 버튼을 누릅니다.
...
- 채널 맨 위에 고정되는 컨테이너 **안**에 **티켓 닫기** 버튼이 있습니다. 이 버튼은 스태프 역할 보유자와 관리자만 쓸 수 있습니다.
...
| `VERIFY_PANEL_IMAGE_URL` | 인증 패널 안에 넣을 배너 이미지 주소 (없음) |
