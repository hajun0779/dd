...
> **핫픽스 2건 반영됨**
>
> 1. `Theme:103` 에서 나던 `American is not a valid member of "Enum.Font"` —
>    제가 없는 폰트 이름을 적었습니다. Theme 이 로드에 실패하면서 그걸 쓰는 화면이
>    전부 같이 죽었습니다. 이제 대체 폰트 표를 **이름으로 적고 실제로 있는 것만**
>    채웁니다. 로블록스 버전이 달라져도 같은 사고가 안 납니다.
> 2. 확장 알 10종(업화·천상·벚꽃·비전·사이버노바·용·꿈·왕실·혼돈·창세)의 기호와 무늬가
>    실제로는 안 들어가 있었습니다. 그 알들은 별도 표에서 필드를 하나하나 옮겨 만드는데
>    거기에 옮기는 줄이 빠져 있었습니다. 화면에서 무늬 없는 알로 보였을 겁니다.
>
> 두 건 다 **실제로 실행해 보고** 찾았습니다 — 로블록스 API 를 흉내 낸 셸에
> 설정 모듈들을 물려 돌리는 검사를 만들어서, 없는 Enum 이름이면 스튜디오와 똑같이
> 에러가 나게 했습니다. 파싱 검사만으로는 둘 다 안 잡혔습니다.
...
| ID | 이름 | 주력 | 특징 |
|---|---|---|---|
| `Aurora` | 오로라 | 하늘 | 하늘에 걸린 Beam 커튼 + 발밑에서 솟는 빛가루 |
| `Storm` | 폭풍우 | 주변 | 눕힌 빗줄기 + 하늘에 실제 파트로 서는 번개 |
| `Meteor` | 유성우 | 맵 | **진짜 파트가 떨어짐**, 착탄 근처만 흔들림 |
| `Sakura` | 벚꽃 바람 | 주변 | 저항 큰 꽃잎이 머뭇거리며 흩날림, 카메라 정지 |
| `Disco` | 디스코 나이트 | 보정 | 색 순환 + 회전 스포트라이트 + 바닥 빛알 |
| `Frost` | 혹한 | 주변 | 옆에서 들이치는 눈보라, 안개 30~320 |
| `Rift` | 대균열 | 맵 | 바닥이 갈라지고 불티가 솟음, 낮은 진동 지속 |
| `GoldRain` | 황금비 | 주변 | 빙글빙글 도는 금화 |
| `Nebula` | 성운 | 하늘 | 궤도 도는 빛구슬 + 느리게 떠다니는 별가루 |
| `Taco` | 타코 파티 | 보정 | 색종이 + 스포트라이트 + 위 노래 |

### 전부 3D 입니다

처음에는 꽃잎·금화·번개를 **화면 위에 GUI 사각형으로** 그렸습니다. 그건 월드에서
일어나는 일이 아니라 화면에 씌운 필터로 보입니다 — 원근도 없고, 건물 뒤로 가려지지도
않고, 고개를 돌려도 같은 자리에 붙어 있습니다. 그래서 전부 진짜 3D 로 바꿨습니다.

```
Local   내 카메라를 따라다니는 3D 방출기
        비·눈·꽃잎·금화·불티. 이 화면에만 있고 서버로 복제되지 않는다.
World   서버가 맵에 놓는 3D 물체
        운석·균열·조명·오로라 커튼. 모두가 같은 것을 본다.
Sky     하늘·안개·밝기
Grade   블룸·대비·채도 (렌더된 3D 장면의 보정이지 덧그리는 게 아니다)
Camera  흔들림·기울기
```

**Local** — 보이지 않는 방출판을 카메라 위(또는 아래)에 하나 두고 그게 카메라를
따라다닙니다. 이러면 가까운 꽃잎은 크고 빠르게, 먼 꽃잎은 작고 느리게 지나가고,
거위와 건물 뒤로 가려지고, 고개를 돌리면 다른 각도에서 보입니다.
방출판은 `Workspace.CurrentCamera` 밑에 둡니다 — 카메라 밑에 있는 것은 그 화면에만
존재하고 복제되지 않아서, 사람이 몇이든 서버 비용이 0 입니다.

**번개**도 3D 입니다. 하늘에 실제 파트로 갈래를 세우고 그 끝에 빛을 달아 주변 지형이
실제로 밝아지게 합니다. 어느 쪽에서 쳤는지가 눈에 보입니다.

**오로라**는 `Beam` 으로 만든 진짜 커튼입니다. 하늘 높이 부착점 두 개를 멀찍이
떨어뜨리고 폭을 크게 줘서 천 한 장으로 만든 뒤, `CurveSize` 를 왔다갔다 시켜
바람에 흔들리게 합니다. 파티클로는 이 그림이 안 나옵니다 — 알갱이는 아무리 겹쳐도
"뿌옇다" 가 되지 커튼이 되지 않습니다.

화면에 남는 2D 는 **연출 이름 표지 하나뿐**이고, 3초 뒤 사라집니다.
